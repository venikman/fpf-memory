...
| `get_fpf_index_status` | Inspect whether the runtime index exists and is fresh against the configured source. | `getFpfIndexStatusInputSchema`: no parameters (stray compatibility args ignored) | `runtimeStatusSchema`: `sourcePath`, `sourceHash?`, `builtAt?`, `snapshotExists`, `currentSourceHash`, `fresh`, `compilerMode`, `artifacts`, `sessionCache` | Use as the first reliance gate before trust-sensitive FPF lookup. |
