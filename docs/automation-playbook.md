...
The repository secrets below gate the automation. Their values appear nowhere in the repo — this ledger records what each one is for, what breaks when it dies, and how to renew it. Expiry is not tracked by hand: every weekly metrics review probes each ledgered secret read-only against `GET /v5/user/tokens/current` and raises a finding when a token is dead, missing, or expiring within 30 days. That weekly finding is the only expiry alarm these tokens have.
...
1. Mint the replacement in the Vercel dashboard (Account Settings → Tokens), scoped to `venikmans-projects`, with an explicit expiration — prefer one year over "no expiration" so the weekly ledger can warn before death instead of never.
2. `gh secret set <NAME> --repo venikman/fpf-memory` with the new value. Values are never committed anywhere.
3. Dispatch the consuming workflow (`gh workflow run vercel-spend-monitor.yml`, or `weekly-metrics.yml` at default inputs) and confirm the run is green and the ledger section shows the new expiry.
4. Revoke the old token in the dashboard.

Registrar and billing lifelines — nothing in CI can see these, so they are recorded here where an absence plan can find them:

- `fpf.sh` is registered at Name.com, paid through **2027-05-09**. Auto-renew must be verified ON in the registrar dashboard (not verifiable from this repo). If the domain lapses, both public surfaces and the hosted MCP endpoint go dark regardless of anything on the GitHub or Vercel side.
- Vercel Pro billing: a failed charge starts a **14-day grace period**, after which all deployments pause, and paused projects require manual resume from the dashboard. The spend monitor does not see billing state; keeping the card current is a human responsibility.
...
3. `bun run smoke:production -- --format markdown --fail-on-breach` and `bun run bench:mcp:qa -- --name mcp-production --url https://mcp.fpf.sh/api/mcp/fpf_reference/mcp --format markdown` — independent post-alias verification that the canonical domains serve the new behavior, not just that a deployment exists. Keep `--fail-on-breach`: without it the smoke command exits zero even when it reports a breach.
...
3. Read runtime and build logs for that deployment — via the Vercel MCP evidence loop above (read-only) or a historical query such as `npx --yes vercel@54.7.1 logs --deployment <deployment-url> --status-code 5xx --since 1h --expand --scope "$FPF_VERCEL_SCOPE"` — to find the failing route and error shape. In the pinned CLI, `logs` queries request history by default and streams live only with `--follow`, so the already-failed requests are readable after the fact; narrow with `--query`, `--level`, or `--request-id` as needed.
...
1. Identify the two rollback targets separately — they can differ after an alias-only hotfix, and `deploy:prod` records them as distinct values. Project production target: `npx --yes vercel@54.7.1 ls fpf-reference-mcp --environment production --status READY --scope "$FPF_VERCEL_SCOPE"` (or `fpf-sh`); the environment and status filters match how `deploy:prod` records this target, and without them previews, failed builds, or in-progress deployments can masquerade as "last good". Canonical-domain target: the deployment the domain served before the incident, from the deploy evidence packet (recorded there via `vercel inspect` of the domain). The bad deployment is itself the newest `READY` production entry in the `ls` output — skip it and take the previous entry, or trust the evidence packet's recorded targets over the listing.
2. Restore the project production target: `npx --yes vercel@54.7.1 promote <previous-project-production-url> --yes --local-config vercel.mcp.json --scope "$FPF_VERCEL_SCOPE"` (website: `vercel.json`) — this mirrors the automatic rollback in `deploy:prod`, which restores project production first so the bad deployment does not stay the project's production target and become the next recorded rollback target.
3. Restore the canonical domain: `npx --yes vercel@54.7.1 alias set <previous-domain-deployment-url> mcp.fpf.sh --scope "$FPF_VERCEL_SCOPE"` (or `fpf.sh`) — canonical domains are aliased explicitly in this repo, so users are back on the known-good deployment only once the alias moves. Use the domain's own recorded target here, not the project production URL from step 2.
4. `bun run smoke:production -- --format markdown --fail-on-breach` — prove the rollback restored user-visible behavior with a command that actually fails on breach; do not stop at the CLI reporting success.
...
- `workflow-keepalive.yml` runs weekly (Wednesday 03:23 UTC) so the schedule-driven fleet above survives GitHub's 60-day rule: in a public repository, GitHub automatically disables every scheduled workflow after 60 days without repository activity, and this repo's only routine activity is sync merges — which stop exactly when upstream goes quiet or a sync PR is stuck on red CI, taking the monitors and their alarms down together. Each run re-enables every active or auto-disabled workflow under `.github/workflows/` via the REST enable endpoint (resetting each inactivity clock and curing an already-fired disable) while leaving manually disabled workflows alone, and — when no branch has been pushed for 45 days — refreshes a marker file on the `automation/keepalive` branch (never `main`, which would run the full CI pipeline on a timestamp). Its own failure opens `Workflow keepalive is failing — scheduled automation may go dark`, closed automatically on the next clean run.
- `weekly-metrics.yml` runs Mondays 06:00 UTC with `bun run weekly:metrics` and publishes one rolling `Weekly metrics review <ISO week>` issue — the ledger for freshness vs the drift SLO (verdict reused from the sync monitor), main-branch activity (sync publications vs other work), Vercel deployments, and Web Analytics visitors/pageviews with week-over-week deltas for `fpf-sh` and `fpf-reference-mcp`, plus the MCP tool-usage report (`bun run usage:report`) embedded as a trailing-24h sample because Vercel runtime-log retention is much shorter than a week. Every data source degrades to an explicit finding (`not_enabled`, `config_error`, `unparsed`, `unavailable`) rather than a crash or a silent blank. Only a default-window run on `main` may write the issue; branch or custom-window dispatches keep the full report in the run's step summary. It enables Web Analytics idempotently (`vercel project web-analytics`); its token chain is `VERCEL_WEEKLY_METRICS_TOKEN` → `VERCEL_SPEND_MONITOR_TOKEN` → `VERCEL_TOKEN`, which today resolves to the spend-monitor token because `VERCEL_WEEKLY_METRICS_TOKEN` has never been created (see the credential and renewal ledger above). Each run also probes every ledgered repo secret against `GET /v5/user/tokens/current` and raises a finding when a token is dead, missing, or expiring within 30 days. Prior-week issues are closed as superseded, so the open issue is always the current review.
