...
import { normalizeForLookup, unique } from '../runtime/text.js';
...
function appendNodeIdList(
  ...
  nodeIds.forEach((nodeId, index) => {
    ...
    lines.push(`${prefix} ${formatNodeReference(snapshot, nodeId, true)}`);
    if (!ordered && index === nodeIds.length - 1) {
      return;
    }
  });
}
