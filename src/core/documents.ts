 ...
 * Generated once by `docs:generate` and reused for navigation and exact-ID
 * search so `route:<slug>` queries can resolve the canonical page even when
 * FlexSearch's substring index doesn't return it (R4-P1-002, R4-P1-003).
 *
 * Written to `src/docs/generated-search-id-registry.ts`, which both the
 * Rspress config and bundled search hook import.
 */
export interface SearchIdRegistry {
  /** Canonical pattern pages keyed by exact pattern ID. */
  patterns: Array<{ id: string; title: string; staticPath: string; group: string }>;
...
export function buildDocsNavigation(snapshot: Snapshot): DocsNavigation {
  return buildDocsNavigationFromRegistry(buildSearchIdRegistry(snapshot));
}

/** Build the sidebar without retaining the full compiler snapshot in Rspress. */
export function buildDocsNavigationFromRegistry(
  registry: SearchIdRegistry,
): DocsNavigation {
  const patternGroups = new Map<string, DocsNavLink[]>();
  for (const pattern of registry.patterns) {
    const bucket = patternGroups.get(pattern.group) ?? [];
    bucket.push({
      text: `${pattern.id} ${pattern.title}`,
      link: pattern.staticPath,
    });
    patternGroups.set(pattern.group, bucket);
  }
...
export function buildSearchIdRegistry(snapshot: Snapshot): SearchIdRegistry {
  const patterns = sortedPatterns(snapshot).map((pattern) => ({
    ...
    group: pattern.part ?? pattern.cluster ?? 'Patterns',
  }));
...
/**
 * Render the search-ID registry as a TypeScript module string suitable for
 * writing to `src/docs/generated-search-id-registry.ts`. Generated by
 * `scripts/generate-docs.ts` (`bun run docs:generate`) so the config and
 * search hook consume the same deterministic projection.
 */
...
// Regeneration runs at \`bun run docs:generate\`.
...
function renderPatternCatalogMarkdown(
  ...
  const lines = [
    ...
    '',
    'New here? Start at the [orientation page](/) — work packets, glossary, and the right entry point per task. Use this catalog when you need to audit the full FPF reference, open an exact ID, or compare neighboring patterns by Part.',
    '',
    '## What this page is',
    '',
