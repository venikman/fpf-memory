...
import { buildDocsNavigation, buildSearchIdRegistry } from './src/core/documents.js';
...
// Search-hook registry — written to a TS module the hook imports so the
// bundled client search code has the canonical ID/title/path table at
// runtime. Regenerated on every config load so it always matches the
// current spec snapshot. Kept in src/docs (alongside search-hooks.ts)
// so the bundler resolves it through the normal TS module graph.
...
writeFileSync(
  searchIdRegistryPath,
  `// Auto-generated by rspress.config.ts — do not edit by hand.
// Regenerated from the FPF spec snapshot on every docs build.
