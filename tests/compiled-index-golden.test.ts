...
describe('compiled index golden snapshot', () => {
  ...
  it('produces a compiled snapshot that satisfies structural invariants', async () => {
    ...
    const canonical: Record<string, { kind: CompiledNode['kind']; titlePrefix: string; part: string }> = {
      ...
      // Upstream (2026-08 sync, ref 72222c13) renamed A.2.1 from
      // "U.RoleAssignment - System Role Assignment" to
      ...
      // "U.RoleStateGraph" to "RoleStateRelation@BoundedContext"; the 2026-08
      // sync (ref 72222c13) renamed it again to
      // "SystemRoleAssignmentStateRelation - Assignment-State Recognition and
      // Work Admission".
      'A.2.5': {
        kind: 'pattern',
